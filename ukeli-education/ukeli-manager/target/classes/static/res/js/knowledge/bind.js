...
var bindSubjectTemplate ='{{each list as subject index}}'+
    ...
    '        <a class="btn btn-minier btn-default" href="javascript:bind({{subject.id}},\'{{subject.ext_vtype}}\')" title="绑定"><i class="ace-icon fa fa-bolt"></i>绑定</a>'+
    '       </div>' +
    '   </td>'+
    '</tr>'+
    '{{/each}}'+
 ...
 * @param id
 */
function bind(subjectId,vtypestr) {
	var substr='自测';
	console.info(knowledgeId+'------'+subjectId+'-----'+vtypestr);
	if(vtypestr.indexOf(substr) >= 0){
		Eymjs.dialog.showUrl('绑定试题',Eymjs.ukeli.url.knowledge.BINDADDSCORE,{knowledgeId:knowledgeId,subjectId:subjectId},500,300);
	}else{
