...
var knowledgeTemplate ='{{each list as knowledge index}}'+
   ...
   // '   <td class="hidden-xs">{{#slock_staus(knowledge.slock,knowledge.id)}}</td>'+
    ...
    '           {{#show(knowledge.id)}}{{#operate(knowledge.id)}}' +
...
function page(page){
    ...
    var typeid = $("#typeidForSearch").val();
    var status = $("#statusForSearch").val();
    Eymjs.page.show(Eymjs.ukeli.url.knowledge.LIST,{keyword:keyword,typeid:typeid,status:status,page:page},knowledgeTemplate,function () {
