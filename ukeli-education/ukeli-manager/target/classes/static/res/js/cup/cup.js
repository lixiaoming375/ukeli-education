...
var cupTemplate ='{{each list as cup index}}'+
    ...
    '   <td class="hidden-xs">{{cup.ext_cuptype}}</td>'+
    ...
    '           {{#show(cup.id)}}{{#operate(cup.id)}}' +
    '           <a class="btn btn-minier btn-pink"  href="javascript:showForBind({{ cup.id}})" title="绑定题目"><i class="ace-icon fa fa-bolt"></i>绑定题目</a>'+
    '       </div>' +
    '   </td>'+
    '</tr>'+
    '{{/each}}'+
...
function reSearch() {
    $("#keyword").val('');
    $("#cuptypeidForSearch").val('');
    $("#statusForSearch").val('-1');
    search();
}
...
function page(page){
    ...
    var cuptypeid=$("#cuptypeidForSearch").val();
    var status=$("#statusForSearch").val();
    Eymjs.page.show(Eymjs.ukeli.url.cup.LIST,{keyword:keyword,cuptypeid:cuptypeid,status:status,page:page},cupTemplate,function () {
