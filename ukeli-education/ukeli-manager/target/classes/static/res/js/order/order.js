...
var orderTemplate ='{{each list as order index}}'+
    ...
    '       </label>'+
    '   </td>'+
    '   <td class="center">{{ order.iorder}}</td>'+
 ...
 * 展示排序的编号
 */
template.helper('iorder',function (id) {
    var html ='<a href="#" onclick="iorder('+id+',1);"><i class="blue ace-icon fa fa-arrow-circle-up bigger-200"></i></a>&nbsp;&nbsp;';
        html +='<a href="#" onclick="iorder('+id+',2);"><i class="blue ace-icon  fa fa-arrow-circle-down bigger-200"></i></a>';
    return html;
});
...
function page(page){
    ...
    Eymjs.page.show(
    		...
    		{username:username,ordernumber:ordernumber,reservationtime:reservationtime,productid:productid,paystatus:paystatus,status:status,page:page},
            orderTemplate
            );
}
 ...
 */
function addorder() {
    Eymjs.dialog.showUrl('订单产品增加',Eymjs.ukeli.url.order.EDIT,{},750,600);
}
/**
 * 修改一条记录
 * @param id
 */
function edit(id) {
    Eymjs.dialog.showUrl('修改订单产品',Eymjs.ukeli.url.order.EDIT,{id:id},750,600);
}
/**
 * 修改按钮
 */
 ...
 */
function show(id){
	 Eymjs.dialog.showUrl('查看',Eymjs.ukeli.url.order.SHOW,{id:id},750,550);
}
 ...
 * @param type
 */
function iorder(id,type) {
    Eymjs.page.online(Eymjs.ukeli.url.order.ORDER,{id:id,type:type},'您确定要调整排序吗？');
}
