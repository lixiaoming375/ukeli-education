...
public class SubjectVideoRestController  implements RestBaseController<SubjectVideoDo> {

	private final static Logger LOGGER =LogManager.getLogger(SubjectVideoRestController.class);
	@Autowired
	...
	public ErrorInfo<SubjectVideoDo> del(String ids, HttpServletRequest request, HttpServletResponse response) throws Exception {
		ErrorInfo<SubjectVideoDo> errorInfo = new ErrorInfo<>();
        try{
            LOGGER.info("需要删除的id=" + ids);
            String[] str = StringUtils.split(ids, ",");
            subjectvideoService.delByIds(str);
	...
	public Page<SubjectVideoDo> list(HttpServletRequest request, HttpServletResponse response) throws Exception {
		Page<SubjectVideoDo> page = null;
        try{
            ...
            page = subjectvideoService.getPage(keyword,status,slock,start,limit);
            if(null != page){
	...
	public ErrorInfo<SubjectVideoDo> save(HttpServletRequest request, HttpServletResponse response) throws Exception {
		ErrorInfo<SubjectVideoDo> errorInfo = new ErrorInfo<>();
        try{
            ...
            */
            ...
            if(id > 0) {
            	...
            	subjectvideoService.update(map, id);
                errorInfo.setSuccess(true);
                errorInfo.setErrorinfo("恭喜您，更新成功");
            }else {
            	SubjectVideoDo subjectvideoDo = RequestUtil.form(SubjectVideoDo.class,request);
            	if(null !=subjectvideoDo) {
            		subjectvideoDo.setAdderid(adminid);
            		subjectvideoDo.setAdder(adminName);
            		subjectvideoService.save(subjectvideoDo);
                    errorInfo.setSuccess(true);
	...
	public ErrorInfo<SubjectVideoDo> status(HttpServletRequest request, HttpServletResponse response) throws Exception {
		ErrorInfo<SubjectVideoDo> errorInfo = new ErrorInfo<>();
        try{
        	long id =RequestUtil.getLong("id", request);
            ...
            if (type==0) {
            	subjectvideoService.updateByItem("status", status, id);
                errorInfo.setSuccess(true);
                errorInfo.setErrorinfo("更新状态成功");
			}else {
				subjectvideoService.updateByItem("auditstatus", status, id);
                errorInfo.setSuccess(true);
	...
	@ResponseBody
	public ErrorInfo<SubjectVideoDo> order(HttpServletRequest request, HttpServletResponse response) throws Exception {
		ErrorInfo<SubjectVideoDo> errorInfo = new ErrorInfo<>();
        try{
        	long id =RequestUtil.getLong("id", request);
        	int type =RequestUtil.getInt("type", request);
        	...
        	SubjectVideoDo subjectvideoDo = subjectvideoService.get(id);
        	if(null ==subjectvideoDo){
        		errorInfo.setSuccess(false);
            	errorInfo.setErrorinfo("对不起，参数错误!");
            	return errorInfo;
        	}
        	SubjectVideoDo vsubjectvideoDo = subjectvideoService.getByOrderType(subjectvideoDo.getIorder(),type);
        	if(null == vsubjectvideoDo){
        		if(type ==1){
                	errorInfo.setSuccess(false);
        	...
        	if(null != subjectvideoDo && null != vsubjectvideoDo){
        		subjectvideoService.order(subjectvideoDo,vsubjectvideoDo);
        	}
        	errorInfo.setSuccess(true);
