...
public class UserSubjectWrongController  extends BaseControllerImpl implements WebBaseController<UserSubjectWrongDo> {

	private final static Logger LOGGER =LogManager.getLogger(UserSubjectWrongController.class);
	@Autowired
	...
	public String edit(ModelMap map, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(id > 0){
            usersubjectwrong = usersubjectwrongService.get(id);
            if(null == usersubjectwrong){
                usersubjectwrong = new UserSubjectWrongDo();
            }else{
                edit=true;
            }
        }
        map.put("usersubjectwrong",usersubjectwrong );
        map.put("edit",edit);
		setting(map,request,response);
        return "/usersubjectwrong/edit";
	}
	...
	public String show(ModelMap map, HttpServletRequest request, HttpServletResponse response) throws Exception {
		Long id = RequestUtil.getLong("id", request);
		UserSubjectWrongDo usersubjectwrong = usersubjectwrongService.get(id);
		map.put("usersubjectwrong",usersubjectwrong );
		setting(map, request, response);
		return "/usersubjectwrong/show";
	}
