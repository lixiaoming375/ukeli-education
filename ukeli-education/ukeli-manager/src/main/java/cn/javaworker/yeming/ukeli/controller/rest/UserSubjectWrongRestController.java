...
*/
@RestController
@RequestMapping("/usersubjectwrong/")
public class UserSubjectWrongRestController  implements RestBaseController<UserSubjectWrongDo> {

	private final static Logger LOGGER =LogManager.getLogger(UserSubjectWrongRestController.class);
	@Autowired
	private UserSubjectWrongService usersubjectwrongService;
	...
	public ErrorInfo<UserSubjectWrongDo> del(String ids, HttpServletRequest request, HttpServletResponse response) throws Exception {
		ErrorInfo<UserSubjectWrongDo> errorInfo = new ErrorInfo<>();
        try{
            LOGGER.info("需要删除的id=" + ids);
            String[] str = StringUtils.split(ids, ",");
            usersubjectwrongService.delByIds(str);
	...
	public Page<UserSubjectWrongDo> list(HttpServletRequest request, HttpServletResponse response) throws Exception {
		Page<UserSubjectWrongDo> page = null;
        try{
            ...
            page = usersubjectwrongService.getPage(kaywordMap,status,slock,start,limit);
            if(null != page){
	...
	public ErrorInfo<UserSubjectWrongDo> save(HttpServletRequest request, HttpServletResponse response) throws Exception {
		ErrorInfo<UserSubjectWrongDo> errorInfo = new ErrorInfo<>();
        try{
            ...
            */
            ...
            if(id > 0) {
            	...
            	usersubjectwrongService.update(map, id);
                errorInfo.setSuccess(true);
                errorInfo.setErrorinfo("恭喜您，更新成功");
            }else {
            	UserSubjectWrongDo usersubjectwrongDo = RequestUtil.form(UserSubjectWrongDo.class,request);
            	if(null !=usersubjectwrongDo) {
            		usersubjectwrongDo.setAdderid(adminid);
            		usersubjectwrongDo.setAdder(adminName);
            		usersubjectwrongService.save(usersubjectwrongDo);
                    errorInfo.setSuccess(true);
	...
	public ErrorInfo<UserSubjectWrongDo> status(HttpServletRequest request, HttpServletResponse response) throws Exception {
		ErrorInfo<UserSubjectWrongDo> errorInfo = new ErrorInfo<>();
        try{
        	long id =RequestUtil.getLong("id", request);
            ...
            if (type==0) {
            	usersubjectwrongService.updateByItem("status", status, id);
                errorInfo.setSuccess(true);
                errorInfo.setErrorinfo("更新状态成功");
			}else {
				usersubjectwrongService.updateByItem("auditstatus", status, id);
                errorInfo.setSuccess(true);
	...
	@ResponseBody
	public ErrorInfo<UserSubjectWrongDo> order(HttpServletRequest request, HttpServletResponse response) throws Exception {
		ErrorInfo<UserSubjectWrongDo> errorInfo = new ErrorInfo<>();
        try{
        	long id =RequestUtil.getLong("id", request);
        	int type =RequestUtil.getInt("type", request);
        	...
        	UserSubjectWrongDo usersubjectwrongDo = usersubjectwrongService.get(id);
        	if(null ==usersubjectwrongDo){
        		errorInfo.setSuccess(false);
            	errorInfo.setErrorinfo("对不起，参数错误!");
            	return errorInfo;
        	}
        	UserSubjectWrongDo vusersubjectwrongDo = usersubjectwrongService.getByOrderType(usersubjectwrongDo.getIorder(),type);
        	if(null == vusersubjectwrongDo){
        		if(type ==1){
                	errorInfo.setSuccess(false);
        	...
        	if(null != usersubjectwrongDo && null != vusersubjectwrongDo){
        		usersubjectwrongService.order(usersubjectwrongDo,vusersubjectwrongDo);
        	}
        	errorInfo.setSuccess(true);
