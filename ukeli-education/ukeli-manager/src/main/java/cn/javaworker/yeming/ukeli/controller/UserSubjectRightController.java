...
public class UserSubjectRightController  extends BaseControllerImpl implements WebBaseController<UserSubjectRightDo> {

	private final static Logger LOGGER =LogManager.getLogger(UserSubjectRightController.class);
	@Autowired
	...
	public String edit(ModelMap map, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(id > 0){
            usersubjectright = usersubjectrightService.get(id);
            if(null == usersubjectright){
                usersubjectright = new UserSubjectRightDo();
            }else{
                edit=true;
            }
        }
        map.put("usersubjectright",usersubjectright );
        map.put("edit",edit);
		setting(map,request,response);
        return "/usersubjectright/edit";
	}
	...
	public String show(ModelMap map, HttpServletRequest request, HttpServletResponse response) throws Exception {
		Long id = RequestUtil.getLong("id", request);
		UserSubjectRightDo usersubjectright = usersubjectrightService.get(id);
		map.put("usersubjectright",usersubjectright );
		setting(map, request, response);
		return "/usersubjectright/show";
	}
