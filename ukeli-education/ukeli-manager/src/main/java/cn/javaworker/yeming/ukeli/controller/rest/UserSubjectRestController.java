...
public class UserSubjectRestController  implements RestBaseController<UserSubjectDo> {

	private final static Logger LOGGER =LogManager.getLogger(UserSubjectRestController.class);
	@Autowired
	...
	public ErrorInfo<UserSubjectDo> del(String ids, HttpServletRequest request, HttpServletResponse response) throws Exception {
		ErrorInfo<UserSubjectDo> errorInfo = new ErrorInfo<>();
        try{
            LOGGER.info("需要删除的id=" + ids);
            String[] str = StringUtils.split(ids, ",");
            usersubjectService.delByIds(str);
	...
	public Page<UserSubjectDo> list(HttpServletRequest request, HttpServletResponse response) throws Exception {
		Page<UserSubjectDo> page = null;
        try{
            ...
            String subjectname=RequestUtil.getString("subjectname", request);
            ...
            page = usersubjectService.getPage(username,subjectname,isdone,isadd,status,slock,start,limit);
            if(null != page){
	...
	public ErrorInfo<UserSubjectDo> save(HttpServletRequest request, HttpServletResponse response) throws Exception {
		ErrorInfo<UserSubjectDo> errorInfo = new ErrorInfo<>();
        try{
            ...
            */
            ...
            if(id > 0) {
            	...
            	usersubjectService.update(map, id);
                errorInfo.setSuccess(true);
                errorInfo.setErrorinfo("恭喜您，更新成功");
            }else {
            	UserSubjectDo usersubjectDo = RequestUtil.form(UserSubjectDo.class,request);
            	if(null !=usersubjectDo) {
            		usersubjectDo.setAdderid(adminid);
            		usersubjectDo.setAdder(adminName);
            		usersubjectService.save(usersubjectDo);
                    errorInfo.setSuccess(true);
	...
	public ErrorInfo<UserSubjectDo> status(HttpServletRequest request, HttpServletResponse response) throws Exception {
		ErrorInfo<UserSubjectDo> errorInfo = new ErrorInfo<>();
        try{
        	long id =RequestUtil.getLong("id", request);
            ...
            if (type==0) {
            	usersubjectService.updateByItem("status", status, id);
                errorInfo.setSuccess(true);
                errorInfo.setErrorinfo("更新状态成功");
			}else {
				usersubjectService.updateByItem("auditstatus", status, id);
                errorInfo.setSuccess(true);
	...
	@ResponseBody
	public ErrorInfo<UserSubjectDo> order(HttpServletRequest request, HttpServletResponse response) throws Exception {
		ErrorInfo<UserSubjectDo> errorInfo = new ErrorInfo<>();
        try{
        	long id =RequestUtil.getLong("id", request);
        	int type =RequestUtil.getInt("type", request);
        	...
        	UserSubjectDo usersubjectDo = usersubjectService.get(id);
        	if(null ==usersubjectDo){
        		errorInfo.setSuccess(false);
            	errorInfo.setErrorinfo("对不起，参数错误!");
            	return errorInfo;
        	}
        	UserSubjectDo vusersubjectDo = usersubjectService.getByOrderType(usersubjectDo.getIorder(),type);
        	if(null == vusersubjectDo){
        		if(type ==1){
                	errorInfo.setSuccess(false);
        	...
        	if(null != usersubjectDo && null != vusersubjectDo){
        		usersubjectService.order(usersubjectDo,vusersubjectDo);
        	}
        	errorInfo.setSuccess(true);
