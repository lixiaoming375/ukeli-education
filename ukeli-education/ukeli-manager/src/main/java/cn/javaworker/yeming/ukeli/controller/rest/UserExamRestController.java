...
public class UserExamRestController  implements RestBaseController<UserExamDo> {

	private final static Logger LOGGER =LogManager.getLogger(UserExamRestController.class);
	@Autowired
	...
	public ErrorInfo<UserExamDo> del(String ids, HttpServletRequest request, HttpServletResponse response) throws Exception {
		ErrorInfo<UserExamDo> errorInfo = new ErrorInfo<>();
        try{
            LOGGER.info("需要删除的id=" + ids);
            String[] str = StringUtils.split(ids, ",");
            userexamService.delByIds(str);
	...
	public Page<UserExamDo> list(HttpServletRequest request, HttpServletResponse response) throws Exception {
		Page<UserExamDo> page = null;
        try{
            ...
            short slock =RequestUtil.getShort("slock", request);
            ...
            page = userexamService.getPage(kaywordMap,status,slock,start,limit);
            if(null != page){
	...
	public ErrorInfo<UserExamDo> save(HttpServletRequest request, HttpServletResponse response) throws Exception {
		ErrorInfo<UserExamDo> errorInfo = new ErrorInfo<>();
        try{
            ...
            */
            ...
            if(id > 0) {
            	...
            	userexamService.update(map, id);
                errorInfo.setSuccess(true);
                errorInfo.setErrorinfo("恭喜您，更新成功");
            }else {
            	UserExamDo userexamDo = RequestUtil.form(UserExamDo.class,request);
            	if(null !=userexamDo) {
            		userexamDo.setAdderid(adminid);
            		userexamDo.setAdder(adminName);
            		userexamService.save(userexamDo);
                    errorInfo.setSuccess(true);
	...
	public ErrorInfo<UserExamDo> status(HttpServletRequest request, HttpServletResponse response) throws Exception {
		ErrorInfo<UserExamDo> errorInfo = new ErrorInfo<>();
        try{
        	long id =RequestUtil.getLong("id", request);
            ...
            if (type==0) {
            	userexamService.updateByItem("status", status, id);
                errorInfo.setSuccess(true);
                errorInfo.setErrorinfo("更新状态成功");
			}else {
				userexamService.updateByItem("auditstatus", status, id);
                errorInfo.setSuccess(true);
	...
	@ResponseBody
	public ErrorInfo<UserExamDo> order(HttpServletRequest request, HttpServletResponse response) throws Exception {
		ErrorInfo<UserExamDo> errorInfo = new ErrorInfo<>();
        try{
        	long id =RequestUtil.getLong("id", request);
        	int type =RequestUtil.getInt("type", request);
        	...
        	UserExamDo userexamDo = userexamService.get(id);
        	if(null ==userexamDo){
        		errorInfo.setSuccess(false);
            	errorInfo.setErrorinfo("对不起，参数错误!");
            	return errorInfo;
        	}
        	UserExamDo vuserexamDo = userexamService.getByOrderType(userexamDo.getIorder(),type);
        	if(null == vuserexamDo){
        		if(type ==1){
                	errorInfo.setSuccess(false);
        	...
        	if(null != userexamDo && null != vuserexamDo){
        		userexamService.order(userexamDo,vuserexamDo);
        	}
        	errorInfo.setSuccess(true);
