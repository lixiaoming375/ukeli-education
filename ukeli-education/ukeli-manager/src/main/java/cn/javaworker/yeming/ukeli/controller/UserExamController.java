...
public class UserExamController  extends BaseControllerImpl implements WebBaseController<UserExamDo> {

	private final static Logger LOGGER =LogManager.getLogger(UserExamController.class);
	@Autowired
	...
	public String edit(ModelMap map, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(id > 0){
            userexam = userexamService.get(id);
            if(null == userexam){
                userexam = new UserExamDo();
            }else{
                edit=true;
            }
        }
	...
	public String show(ModelMap map, HttpServletRequest request, HttpServletResponse response) throws Exception {
		Long id = RequestUtil.getLong("id", request);
		UserExamDo userexam = userexamService.get(id);
		map.put("userexam",userexam );
		setting(map, request, response);
		return "/userexam/show";
	}
