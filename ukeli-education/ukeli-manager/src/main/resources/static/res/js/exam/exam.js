...
var examTemplate ='{{each list as exam index}}'+
   ...
   // '   <td class="hidden-xs">{{#order(exam.id)}}</td>'+
    ...
    '           {{#show(exam.id)}}{{#operate(exam.id)}}' +
    '           <a class="btn btn-minier btn-pink"  href="javascript:showForBind({{ exam.id}})" title="绑定题目"><i class="ace-icon fa fa-bolt"></i>绑定题目</a>'+
    '       </div>' +
    '   </td>'+
    '</tr>'+
    '{{/each}}'+
...
function page(page){
   // Eymjs.page.show(Eymjs.ukeli.url.exam.LIST,{keyword:keyword,page:page},examTemplate);
    ...
    Eymjs.page.show(Eymjs.ukeli.url.exam.LIST,{examName:examName,status:status,page:page},examTemplate,function () {
