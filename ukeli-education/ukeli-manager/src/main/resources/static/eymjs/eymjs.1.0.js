    ...
    show:function (url,params,tplid,callback) {
        Eymjs.data.ajax(url,params,function (data) {
             ...
             if(list  && list.length > 0){
                 ...
                 document.getElementById('ajax-data').innerHTML = '<tr ><td colspan="1000">对不起，暂无数据！</td></tr>';
                 Eymjs.page.page(data.totalRecords,data.currentPage,data.pageSize);
             }
        },true);
    },
    /**
...
Eymjs.form = {
    ...
    saveComplateNext :	function (data) {
        if (data.success) {
            layer.closeAll();
            console.info("data.input="+data.input);
            if(data.input>0){
            	Eymjs.toastr.success('恭喜您第一步保存成功');
            	nextedit(data.input);
            }else if(data.flushType==0){
            	Eymjs.toastr.success('恭喜您 保存成功');
                try{search();}catch(e){console.log(e)};
            }else if(data.flushType ==1){
                window.location.reload();
            }
