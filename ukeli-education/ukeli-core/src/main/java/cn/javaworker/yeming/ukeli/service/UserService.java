...
public interface UserService extends BaseService<UserDo>{
	 ...
	 * @return
	 */
	//Page<UserDo> getPage(String keyword,int status,int slock,long start, int limit);
	 ...
	 * @return
	 */
	UserDo getByOrderType(long iorder, int type);
	/**
	 * 处理排序
	 * @param userDo
	 * @param vuserDo
	 */
	void order(UserDo userDo,UserDo vuserDo);


	Page<UserDo> getPage(Map<String, Object> kaywordMap, int status, int slock, long start, int limit);
}