...
public interface TeacherService extends BaseService<TeacherDo>{
	 ...
	 * @return
	 */
	Page<TeacherDo> getPage(String keyword,int status,int slock,long start, int limit);
	 ...
	 * @return
	 */
	TeacherDo getByOrderType(long iorder, int type);
	/**
	 * 处理排序
	 * @param teacherDo
	 * @param vteacherDo
	 */
	void order(TeacherDo teacherDo,TeacherDo vteacherDo);
}